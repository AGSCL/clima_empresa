...
Se decidió imputar el sexo para los casos en que no fue posible imputar el correo manualmente ya que el correo sólo indicaba iniciales, o bien porque la particularidad del nombre hacía difícil inferir un género. Para ello se imputó de vañpres faltantes mediante árboles de decisión aleatorios iterativos (chained random forest) para la variable con valores perdidos.

<br>

```{r paso0,eval=T, echo=T, paged.print=TRUE, eval=T}
#https://rdrr.io/github/domus11move/IMPMUL2/man/missRanger.html
nombres_xlsx <- readxl::read_excel("nombres.xlsx.xlsx") 
colnames(nombres_xlsx)<-c("hash","sexo","correo","sexo_manual_imp","com1","com2")
nombres_xlsx_filt <-
nombres_xlsx%>% dplyr::group_by(hash) %>% dplyr::add_count() %>% dplyr::ungroup() %>% dplyr::filter(n==1)

encuesta_rec_cor<-
encuesta_rec %>% 
  dplyr::left_join(nombres_xlsx_filt, by=c("correo_val_cor"="hash")) %>% 
  dplyr::select(-sexo.x,-sexo.y,-com1,-com2,-n)#%>% dim()
...
library(missRanger)
set.seed(2125)
encuesta_rec_cor_imp <- missRanger(
                encuesta_rec_cor, 
                formula = sexo_manual_imp ~ a_dir + b_ger + c_area,
                num.trees = 400, 
                returnOOB=T,
                maxiter=50000,
                verbose = 2, 
...
encuesta_rec_cor_imp<-dplyr::rename(encuesta_rec_cor_imp,"sexo"="sexo_manual_imp")
...
Posteriormente, se añaden los puntajes estandarizados asociados a cada factor hallado.

<br>

```{r paso1,eval=T, echo=T, paged.print=TRUE, eval=T}
#
meas_model_original_pred_lavaan<-cbind(encuesta_rec_cor_imp,lavPredict(tot_4f_inicial))
```
...
```{r paso0_a,eval=T, echo=T, paged.print=TRUE, eval=T}
rbind.data.frame(broom::tidy(lm(fa1~ a_dir+ b_ger+ sexo,data=meas_model_original_pred_lavaan),conf.int=T),
broom::tidy(lm(fa2~ a_dir+ b_ger+ sexo,data=meas_model_original_pred_lavaan),conf.int=T),
broom::tidy(lm(fa3~ a_dir+ b_ger+ sexo,data=meas_model_original_pred_lavaan),conf.int=T),
broom::tidy(lm(fa4~ a_dir+ b_ger+ sexo,data=meas_model_original_pred_lavaan),conf.int=T)) %>% 
  ...
  kableExtra::group_rows("Identificación, proyección e involucramiento (fa1)",1,28) %>% 
  kableExtra::group_rows("Supervisor inmediato (fa2)",29,56) %>% 
  kableExtra::group_rows("Capacidad desiderativa y valórica de la empresa (fa3)",57,84) %>% 
  kableExtra::group_rows("Formación e Información (fa4)",85,112) %>% 
  kableExtra::scroll_box(width = "100%", height = "375px") 
  
  # col.names = c("Var. Latentes",   )
```

No se ven diferencias por gerencia, dirección o sexo en los puntajes estandarizados para cada factor.

<br>

# Análisis de Clases Latentes

A diferencia del análisis anterior que tiene foco en los ítems, este análisis busca encontrar patrones de respuesta común por parte de los encuestados, y clasificarlos de acuerdo  cómo se distribuyen los datos. Para ello se eligió la dirección para la cual trabajaban, el sexo y el puntaje en cada uno de los factores antes seleccionados, divididos en quintiles de acuerdo: "Identificación, proyección e involucramiento (fa1)", "Supervisor inmediato (fa2)", "Capacidad desiderativa y valórica de la empresa (fa3)", "Formación e Información (fa4)". De tal manera que, por ejemplo, un usuario con bajos puntajes en el factor 1 se encontrará en el primer quintil (20% de puntajes más bajos) de la variable latente fa1 (que hemos denominado Identificación, proyección e involucramiento).
...
```{r fig2_Comparison, echo=T, fig.align='center', fig.pos='H', fig.cap= "Figura 1. Probabilidades de Respuesta a categorías", message=FALSE, error=T, eval=T, fig.height=15}
...
if(no_mostrar==1){
  invisible("Para ver los niveles de variables")
  levels(factor(meas_model_original_pred_lavaan$sexo))
  table(mydata$sexo)
}
...
lcmodel_a2<-
lcmodel_a %>% 
  dplyr::mutate(L2=factor(L2, levels=c("a_dir","sexo","fa1","fa2","fa3","fa4"), ordered=T)) %>% 
  dplyr::mutate(Var2 = factor(dplyr::case_when(as.character(Var2)=="Pr(6)"~"Pr miss",
                                               TRUE~as.character(Var2)))) %>% 
  dplyr::mutate(Var2_lab = factor(dplyr::case_when(L2=="a_dir"&Var2=="Pr(1)"~"Admin. & Fin.",
                                  L2=="a_dir"&Var2=="Pr(2)"~"Gcia. General",
                                  L2=="a_dir"&Var2=="Pr(3)"~"Marketing",
                                  L2=="a_dir"&Var2=="Pr(4)"~"Marketing Clientes",
                                  L2=="a_dir"&Var2=="Pr(5)"~"Op. & Client.",                                                                        L2=="a_dir"&Var2=="Pr(6)"~"RRHH",
                                  L2=="a_dir"&Var2=="Pr(7)"~"Ventas",
                                  L2=="sexo"&Var2=="Pr(1)"~"F",
                                  L2=="sexo"&Var2=="Pr(2)"~"H",
                                  L2=="sexo"&Var2=="Pr(3)"~"NS",
                                  grepl("fa",L2)&Var2=="Pr(1)"~"quintil 1",
                                  grepl("fa",L2)&Var2=="Pr(2)"~"quintil 2",
                                  grepl("fa",L2)&Var2=="Pr(3)"~"quintil 3",
                                  grepl("fa",L2)&Var2=="Pr(4)"~"quintil 4",
                                  grepl("fa",L2)&Var2=="Pr(5)"~"quintil 5",                 
                                               T~as.character(Var2)))) %>% 
  dplyr::mutate(value=round(value, 3),label_text=paste0("Valor= ",Var2_lab,"\nPr= ",scales::percent(value)))
zp1 <- ggplot(lcmodel_a2,aes(x = L2, y = value, fill = Var2, text=label_text))
zp1 <- zp1 + geom_bar(stat = "identity", position = "stack")
zp1 <- zp1 + facet_grid(Var1 ~ .) 
zp1 <- zp1 + scale_fill_brewer(type="seq", palette="Greys", na.value = "white") +theme_bw()
zp1 <- zp1 + labs(y = "% of Probabilidades de Respuesta", 
                  x = "Items",
                  fill ="Cateorías de\nRespuesta")
...
ggplotly(zp1, tooltip = c("label_text"))%>%#
...
- La primera clase (`r round(colMeans(LCA_best_model_final$posterior)*100,2)[1]`%) tiene una distribución similar de hombres y mujeres (alta proporción de mujeres), y en los 4 factores se encuentra un grado de acuerdo estandarizado en torno al quintil 4. Particularmente, en "Identificación, proyección e involucramiento (fa1)" y "Formación e Información (fa4)". En los factores 2 y 3 ("Supervisor inmediato (fa2)" y "Capacidad desiderativa y valórica de la empresa (fa3)") tienen valores inconsistentes (un importante porcentaje con valores en el quintil 1 y 2, versus valores más altos en el quintil 5).
- La segunda clase (`r round(colMeans(LCA_best_model_final$posterior)*100,2)[2]`%) abarca a una menor proporción del eprsonal de ventas, y una leve mayor proporción de personal de Op. & client. y Marketing clientes, mayor proporción de mujeres, que evalúan los factores en torno al quintil 3, con una distribución bastante desigual/inconsistentes en los factores 2 y 3 (con más valores bajos o en quintiles inferiores, y a su vez con valores altos como el quintil 4)
- La tercera clase (`r round(colMeans(LCA_best_model_final$posterior)*100,2)[3]`%) tiene una distribución en el promedio de las distintas variables, aunque con una mayor proporción de personal de Administración y finanzas, evaluando entorno al segundo quintil las preguntas en las distintas dimensiones, siendo la peor evaluada la dimensión sobre "Formación e Información (fa4)", seguido por "Supervisor inmediato (fa2)"
- La cuarta clase (`r round(colMeans(LCA_best_model_final$posterior)*100,2)[4]`%) abarca una leve mayor concentración de personal de ventas, en su mayoría hombres y que todos evalúan muy favorablemente las dimensiones expuestas.
- La quinta clase (`r round(colMeans(LCA_best_model_final$posterior)*100,2)[5]`%) tiene bastantes personas en dirección de Op. & client., en su mayoría hombres, y con puntajes más bajos en el grado de acuerdo con enunciados sobre "Identificación, proyección e involucramiento (fa1)", "Supervisor inmediato (fa2)", "Capacidad desiderativa y valórica de la empresa (fa3)" y "Formación e Información (fa4)".
...
```{r fig1_Comparison, echo=T, fig.align='center', fig.pos='H', fig.cap= "Figura 2. Comparative of models of latent classes (2 to 7, from left to right)", message=FALSE, error=T, eval=T, fig.height=10}
...
lc_entropy_table%>%
  ...
  dplyr::filter(indices %in% c('Criterio de Información\nde Akaike',"Criterio de Información\nBayesiano (BIC)","BIC Ajustado")) %>%
  dplyr::mutate(Nclass=factor(Nclass)) %>% 
  ggplot(aes(x=Nclass,y=value,color=indices, group=indices))+
  geom_line()+
  geom_point()+
  theme_classic()+
  labs(x="Número de clases", y="Valores")+
  labs(color = "Indices")+
...
```{r fig3_perfiles, echo=T, fig.align='center', fig.pos='H', fig.cap= "Figura 3. Perfil datos empíricos", message=FALSE, error=T, eval=T, fig.height=10}
bars_probs_lca<-
